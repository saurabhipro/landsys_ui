...
export class StateService {
  ...
  update(state: IState): Observable<EntityResponseType> {
    return this.http.put<IState>(`${this.resourceUrl}/${getStateIdentifier(state) as number}`, state, { observe: 'response' });
  }

  partialUpdate(state: IState): Observable<EntityResponseType> {
    return this.http.patch<IState>(`${this.resourceUrl}/${getStateIdentifier(state) as number}`, state, { observe: 'response' });
  }
