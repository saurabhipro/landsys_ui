...
export class BankBranchComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.bankBranchService
      ...
      .subscribe({
        next: (res: HttpResponse<IBankBranch[]>) => {
  ...
  trackId(_index: number, item: IBankBranch): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IBankBranch[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/bank-branch'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
